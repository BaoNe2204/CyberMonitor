...
export const AuditLogsApi = {
  ...
  getTopUsers: async (days = 30, top = 5) => {
...
export function createSignalRConnection(callbacks: SignalRCallbacks): { connect: () => void; disconnect: () => void } {
  ...
  const connect = () => {
    ...
    try {
      ...
      connection.on('AuditLogReceived', (logDto: AuditLogEntry) => {
        console.log('[SignalR] AuditLogReceived:', logDto.action);
        callbacks.onAuditLogReceived?.(logDto);
      });
